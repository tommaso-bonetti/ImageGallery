...
public class ImageDAO {
	...
	public List<Image> fetchImagesByAlbum(int albumId, int page) throws SQLException {
		List<Image> images = new ArrayList<>();
		
		String query =
				"SELECT * FROM Image WHERE imageId = ANY (SELECT imageId FROM AlbumImages WHERE albumId = ?)"
				+ " ORDER BY uploadDate DESC, imageId DESC LIMIT ?, ?";
		try (PreparedStatement prepStatement = connection.prepareStatement(query)) {
			prepStatement.setInt(1, albumId);
			prepStatement.setInt(2, 5 * (page-1));
			prepStatement.setInt(3, 6);
			try (ResultSet res = prepStatement.executeQuery()) {
				while (res.next()) {
					Image image = new Image();
					image.setId(res.getInt("imageId"));
